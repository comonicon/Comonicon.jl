...
function cmd_script(mod, shadow; exename=PATH.default_exename(), project=PATH.project(mod), sysimg=PATH.default_sysimg(), compile=nothing, optimize=2)
...
function install(mod::Module, name=default_name(mod);
    ...
    shell_script = cmd_script(mod, shadow;
        exename=exename, project=project,
        sysimg=sysimg_path, compile=compile,
        optimize=optimize
    )
